...
public class UserController {
    ...
    public String updateUserProfile(HttpSession session, @RequestParam("phoneNumber") String phoneNumber, @RequestParam("firstName") String firstName, @RequestParam("lastName") String lastName, @RequestParam("email") String email, @RequestParam(value = "newPassword", required = false) String newPass, @RequestParam(value = "confirmPass", required = false) String confirmPass, @RequestParam("address") String address, @RequestParam("currentPass") String currentPass) {
        if (isUserLoggedIn(session)) {
            Long userId = (Long) session.getAttribute(USER_ID_ATTRIBUTE);
            String password = UserManager.findUser(userId).getPassword();
            ...
            UserManager.updateUser(userId, Utilities.fixName(firstName), Utilities.fixName(lastName), phoneNumber.trim(), email.trim().toLowerCase(), newPass, address.trim());
