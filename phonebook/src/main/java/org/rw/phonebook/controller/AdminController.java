...
public class AdminController {
    ...
    public String updateAdminProfile(HttpSession session, @RequestParam("phoneNumber") String phoneNumber, @RequestParam("email") String email, @RequestParam("newPassword") String newPass, @RequestParam("confirmPass") String confirmPass, @RequestParam("currentPass") String currentPass) {
        if (isAdminLoggedIn(session)) {
            Long adminId = (Long) session.getAttribute(ADMIN_ID_ATTRIBUTE);
